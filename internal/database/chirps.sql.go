...
const getAllChirpsOf = `-- name: GetAllChirpsOf :many
SELECT id, created_at, updated_at, body, user_id
FROM chirps
WHERE user_id = $1
ORDER BY created_at ASC
...
func (q *Queries) GetAllChirpsOf(ctx context.Context, userID uuid.UUID) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getAllChirpsOf, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
